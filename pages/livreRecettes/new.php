...
if(!empty($_POST)) {
    ...
    if (empty($errors)) {
        $requete = $pdo->prepare('
        INSERT INTO recette (name, slug, duration, content, created_at)
        VALUES (:name, :slug, :duration, :content, :created_at)
        ');
        $dateTime = new DateTime('');
        $requete->execute(['name' => ucfirst($_POST['titre']), 'slug' => $slug, 'duration' => $_POST['duree'], 'content' => nl2br($_POST['content']), 'created_at' => $dateTime->format('Y-m-d H:i:s')]);
        ...
        for($i=0; $i<count($_POST['typeCat']); $i++) {
            ...
            ');
            $requete->execute(['type_id' => $_POST['typeCat'][$i], 'recette_id' => $recette[0]['id']]);
        }
        ...
        if(isset($_POST['categorie'])) {
            for($i=0; $i<count($_POST['categorie']); $i++) {
                ...
                ');
                $requete->execute(['categorie_id' => $_POST['categorie'][$i], 'recette_id' => $recette[0]['id']]);
            }
        }
        ...
        for($i=0; $i<count($_POST['ingredient']); $i++) {
            ...
            if($_POST['ingredientQuantite'][$i] !== '') {
                $quantite = ($_POST['ingredientQuantite'][$i])/($_POST['nbPersonne']*1);
            } else {
                $quantite = null;
            }
            $requete = $pdo->prepare('
            ...
            ');
            $requete->execute(['quantite' => $quantite, 'unite' => $_POST['ingredientUnite'][$i], 'ingredient_id' => $_POST['ingredient'][$i], 'recette_id' => $recette[0]['id']]);
        }

        header('Location: ' . '/mon-livre-de-recettes' . '?modif=1');
        exit();
...
<h1 class="page-title margin-top-40 cursor-default">Ajouter une nouvelle recette</h1>

<?php if (!empty($errors)): ?>
    <div class="page-recette-alerte-echec cursor-default">
        <h2 class="beige catamaran p-25 bold text-align-center">La recette n'a pas pu être créée</h2>
    </div>
<?php endif ?>
...
<form action="" method="POST">
    ...
    <div class="display-flex flex-direction-column margin-center edit-recette-input-titre-div margin-top-30 cursor-default">
        ...
        <?php else: ?>
            <p class="catamaran edit-recette-input-titre-label js-edit-recette-input-titre-label">Titre</p>
            <input type="text" id="titre" name="titre" placeholder="Titre" value="" class="catamaran noir p-20 edit-recette-input-titre js-edit-recette-input-titre">
        <?php endif?>
    </div>

    <div class="display-flex justify-content-center margin-top-50 cursor-default">
        <?php foreach($categories as $categorie): ?>
            ...
            <div class="display-flex flex-direction-column align-items-center">
                ...
                <p class="margin-leftright-30 catamaran p-20 noir"><?= $categorie['name']?></p>
                <?php if (!empty($errors)): ?>
                    ...
                    <?php else: ?>
                        <div class="page-edit-recette-checkbox-categorie cursor-pointer js-checkbox-categorie margin-top-10"></div>
                        <input type="checkbox" id="<?= $categorie['id']?>" name="categorie[]" value="<?= $categorie['id']?>" class="hidden js-checkbox-categorie-value">
                    <?php endif ?>
                <?php else: ?>
                    <div class="page-edit-recette-checkbox-categorie cursor-pointer js-checkbox-categorie margin-top-10"></div>
                    <input type="checkbox" id="<?= $categorie['id']?>" name="categorie[]" value="<?= $categorie['id']?>" class="hidden js-checkbox-categorie-value">
    ...
    <div class="margin-center width-542 margin-top-70 cursor-default">
        ...
        <?php else :?>
            <div class="display-flex align-items-center">
                <p class="catamaran p-20 bold noir">Préparation:</p>
                <input type="text" name="duree" placeholder="..." value="" class="edit-recette-duree-input catamaran noir p-20">
                <p class="catamaran p-20 noir">minutes</p>
            </div>
        <?php endif ?>
    </div>
    ...
    <?php if ((isset($errors['nbPersonneneg'])) || (isset($errors['nbPersonneint'])) || (isset($errors['quantiteint'])) || (isset($errors['ingredientnull'])) || (isset($errors['ingredientmatch'])) || (isset($errors['ingredientduplica'])) || (isset($errors['quantiteunitematch']))): ?>
        <div class="display-flex flex-direction-column margin-center width-598 margin-top-30 cursor-default">
            ...
            <div class="display-flex align-items-center margin-left-28">
                ...
                <input type="text" id="nbPersonne" name="nbPersonne" placeholder="..." value="<?=$_POST['nbPersonne']?>" class="edit-recette-personnes-input catamaran noir p-20 <?php if((isset($errors['nbPersonneneg'])) || (isset($errors['nbPersonneint']))):?> edit-recette-personnes-input-red <?php endif ?>">
    ...
    <?php else: ?>
        <div class="display-flex flex-direction-column margin-center width-598 margin-top-30 cursor-default">
            <div class="display-flex align-items-center margin-left-28">
                <p class="catamaran p-20 bold noir">Ingrédients:</p>
                <p class="catamaran p-20 noir margin-left-30">Pour</p>
                <input type="text" id="nbPersonne" name="nbPersonne" value="" placeholder="..." class="edit-recette-personnes-input catamaran noir p-20">
                <p class="catamaran p-20 noir">personne(s)</p>
            </div>
    ...
    <div class="background-color-beige margin-top-70 page-recette-div-recette-contenu cursor-default">
        ...
        <div class="margin-top-40">
            ...
            <?php else: ?>
                <textarea name="content" id="content" placeholder="Contenu de la recette" class="catamaran p-20 noir edit-recette-contenu-div js-edit-recette-contenu-div"></textarea>
            <?php endif ?>
        </div>
    </div>
...
<div class="display-flex justify-content-center">
    <a href="<?=$this->router->generate('livreRecettes')?>" class="catamaran p-20 orange">
